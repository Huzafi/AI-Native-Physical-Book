...
const Search = ({ placeholder = "Search the book..." }) => {
  ...
  const addToSearchHistory = useCallback((searchQuery) => {
    ...
    setSearchHistory(prevHistory => {
      // Remove duplicates
      const filteredHistory = prevHistory.filter(item => item.query.toLowerCase() !== searchQuery.toLowerCase());

      // Add new search at the beginning
      const newHistory = [{ query: searchQuery, timestamp: new Date().toISOString() }, ...filteredHistory];
  ...
  const performSearch = async (searchQuery, page = 1) => {
    ...
    try {
      ...
      // Add to search history when a successful search is performed
      ...
      <form onSubmit={handleSubmit} className={styles.searchForm}>
        <div className={styles.searchInputWrapper}>
          <input
            ...
            onBlur={() => {
              // Hide history after a short delay to allow for button clicks
              setTimeout(() => {
            ...
            {searchHistory.map((historyItem, index) => (
              <li key={index} className={styles.historyItem}>
                ...
                >
                  ...
                  <span className={styles.historyQuery}>{historyItem.query}</span>
                </button>
              </li>
            ))}
          </ul>
